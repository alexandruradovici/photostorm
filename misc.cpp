...
char * help_path (const char *dfile)
{
	char *result;
	_ps_config *config = ps_get_config ();
	if (dfile==NULL && config!=NULL) result=strdup ("");
	else
	{
		char t[10000];
		if (config->system.libs[strlen(config->system.libs)-1]!=ps_dir_sep)
		  sprintf (t, "%s%cDocs%c%s", config->system.libs, ps_dir_sep, ps_dir_sep, dfile);
        else
          sprintf (t, "%sDocs%c%s", config->system.libs, ps_dir_sep, dfile);
		result = strdup (t);
	}
	return result;
}
...
const char *generate_security_code (int n)
{
	...
	for (int i=0;i<n/2;i++) 
	{
		...
		sprintf (code, "%s%c", code, v);
	}
	sprintf (code, "%s%ld", code, time(0));
	for (int i=0;i<n/2;i++) 
	{
		int t = ps_random (16);
		char v;
