		...
		if (f!=NULL)
		{
			...
			if (e!=-1)
			{
				...
				if (result!=NULL)
				{
					...
					result->date = (char*)malloc (n_poze);
					//memcpy (&result->date[0], antet, n_antet);
					//result->antet=antet;
					//if (n_poze>0) memcpy (result->date, pozele, n_poze);
					if (pozele!=NULL && b!=0) memcpy (&result->date[0], pozele, n_poze);
